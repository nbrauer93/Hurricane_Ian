...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct  5 21:48:04 2022
...
file = '2A.GPM.DPR.V9-20211125.20220929-S040126-E053357.048778.V07A.HDF5'
...
echo_top = DPR['FS']['PRE']['heightStormTop'][:]/1000

#%%

###Define cross-section length

ind1 = np.where((lon[:,0]>=-82.)) #Where are the DPR lons >= -100
ind2 = np.where((lon[:,0])<=-81.59) #Where are lons <= -85
...
#Now slice to get proper cross section length

#lons_raxpol = lons[0:19]
#lats_raxpol = lats[0:19]
...
m.plot(lons,lats, 'k--', linewidth = 4)
...
plt.title('GPM Overpass 09/29 0503 UTC KuPR (Attenuation-Corrected)', size = 20, y = 1.04)    


x2star,y2star = m(raxpol_lon,raxpol_lat)
m.plot(x2star,y2star,'bo',markersize=10, color = 'b')
...
echo_top[echo_top<0.5] = np.nan



cmin = 0.; cmax = 15.; cint = 0.5; clevs = np.round(np.arange(cmin,cmax,cint),2)
...
m = Basemap(projection='cyl',lon_0=np.mean(xlim),lat_0=np.mean(ylim),llcrnrlat=ylim[0],urcrnrlat=ylim[1],llcrnrlon=xlim[0],urcrnrlon=xlim[1],resolution='i')
m.drawstates(), m.drawcountries(), m.drawcoastlines()
cs = m.contourf(lon,lat,echo_top,clevs,cmap='turbo',extend='both')
cs2 = m.plot(lon[:,0]+0.03,lat[:,0]-0.03,'--k',zorder=4)
cs3 = m.plot(lon[:,-1]-0.03,lat[:,-1]+0.03,'--k',zorder=4)
m.plot(lon[ind3,ray],lat[ind3,ray],'-w',zorder=4,lw=0.25,alpha=0.75)
...
m.drawmeridians(meridians, labels = [False, False, False, True],fontsize = 20)
cbar = m.colorbar(cs,size='2%')
cbar.ax.set_ylabel('[km]',name='Calibri',size=18)
cticks = []
for i in clevs:
    cticks.append(int(i)) if i.is_integer() else cticks.append(i)
    cbar.set_ticks(clevs[::4])
...
plt.title('GPM Overpass 09/29 0503 UTC Echo Top Height', size = 20, y = 1.04)  
...
plt.title(r'GPM Overpass 09/29 0503 UTC $D_{M}$ ', size = 20)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'GPM Overpass 09/29 0503 UTC $log_{10 }(N_{W})$ ', size = 20)
plt.xlim(dist[0],dist[1])
plt.ylim(0,12)
cbar = plt.colorbar()
cbar.ax.tick_params(labelsize = label_size)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'GPM Overpass 09/29 0503 UTC KuPR ', size = 20)
#plt.xlim(300,450)
plt.xlim(dist[0], dist[1])
plt.ylim(0,10)
cbar = plt.colorbar()
...
plt.title('Vertical Slope of KuPR in Liquid Phase 09/29 0503 UTC', size = 22, y = 1.04)
