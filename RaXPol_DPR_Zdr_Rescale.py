...
file = 'RAXPOL-20220929-051136-A154.0-D.nc'
...
zdr = nc.variables['Differential_Reflectivity'][:]-3 #To account for miscalibration bias
...
range_gate = np.zeros_like(zdr[0,:])
...
height = np.ones(zdr.shape)*np.nan
s = np.ones(zdr.shape)*np.nan
...
#Nous alons refaire la grammaire necessaire pour compredendre les mailleurs actions 
  
  
zdr_rescale = np.ones((zdr.shape[0],10))*np.nan
s_rescale = np.ones((zdr.shape[0],10))*np.nan
height_rescale = np.ones((zdr.shape[0],10))*np.nan




for i in range(zdr_rescale.shape[1]):
    ...
    zdr_rescale[:,i] = np.nanmean(zdr[:,(avg_interval*i):(avg_interval*i)+avg_interval], axis = 1)
...
plt.contourf(s_rescale/1000,height_rescale/1000, zdr_rescale, extend = 'both', cmap = 'turbo', levels = np.arange(-1,3,step = 0.1))
    #plt.contourf(s/1000,height/1000, vel, extend = 'both', cmap = 'pyart_NWSVel', levels = np.arange(-16,16,step = 0.5))
cbar = plt.colorbar()
cbar.ax.tick_params(labelsize = label_size)
cbar.set_label(label = r'[dB]',size = label_size)
...
plt.title(r'RaXPol $154^{o}$ Azimuth $Z_{DR}$ 9/29 ' + time + ' UTC', size = label_size)
